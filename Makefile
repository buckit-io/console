...
test-integration:
	...
	@(cd integration && go test -coverpkg=../api -c -tags testrunmain . && mkdir -p coverage &&  ./integration.test -test.v -test.run "^Test*" -test.coverprofile=coverage/system.out $(INTEGRATION_TEST_FLAGS))
...
coverage:
	@(GO111MODULE=on go test -v -coverprofile=coverage.out github.com/buckit-io/console/api/... && go tool cover -html=coverage.out && open coverage.html)
